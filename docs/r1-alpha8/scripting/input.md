...
> The context globals are inputs, not state you own. Assigning to `position_x` or `input_x` does nothing — only variables declared in `props()` are read back. To move an entity, call a transform function such as `translate(x, y, z)` (see [Lua](/docs/r1-alpha8/scripting/lua#moving-and-transforming)).
...
- [Lua](/docs/r1-alpha8/scripting/lua) — the scripting backend
- [Physics](/docs/r1-alpha8/scripting/physics) — forces, impulses, and `PhysicsReadState`
- [Scripting Overview](/docs/r1-alpha8/scripting/overview) — how scripts attach and dispatch by extension
