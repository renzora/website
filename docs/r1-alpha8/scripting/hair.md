...
`renzora_hair` ships as a `cdylib` distribution plugin (same model as ragdoll/cloth/lumen). It is not built into the engine by default — drop the built artifact into `<exe>/plugins/` to enable it (see [Building Plugins](/docs/r1-alpha8/extending/plugins)).
...
> **The sway only runs in Play or Simulate mode.** While editing, the groom is held in its grown rest shape and rides the model rigidly. Use **[Simulate](/docs/r1-alpha8/editor/viewport#simulate-mode)** (the blue flask, in the dropdown beside Play) to watch it move while keeping the editor live.
...
- [Ragdoll Physics](/docs/r1-alpha8/scripting/ragdoll) — solver-based skeletal ragdoll (for pre-rigged bones, not generated strands)
