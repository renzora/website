...
All three honour **Settings → Interface → Display → Scroll Speed**, a multiplier on the scroll rate (1.5 is the default feel) persisted per user in `~/.renzora/settings.toml`.
...
- **Chevron toggle** at the right end of the header in both states — **∨** collapses the open panel, **∧** reopens the collapsed strip. The **∧** restores the height you left it at, so collapsing and reopening gives you back the panel you had.
- **Drag it open**: grab the collapsed strip's background and pull upward — it opens and keeps sizing under your cursor in one gesture. **Drag it closed** the same way: pull the top edge down past its minimum and it snaps shut. Closing it this way leaves **no height to restore** — the last height the drag wrote is the sliver the panel passed through on its way out, not one you picked — so the chevron opens it at the usual 40% afterwards.
...
- **It gets out of the way while you drag an asset out of it.** Drag a file out of the Assets tab (or a shape out of the Shape Library) and the panel slides closed the moment your cursor leaves its top edge, so you can see the viewport, the hierarchy or the inspector slot you're aiming at. Dragging *within* the panel — onto a folder, onto another tab — never triggers it. To bring it back, take the drag **down near the bottom of the editor**, to the collapsed strip or the last inch or so above it. It deliberately isn't the space the panel would occupy: while the panel is closed that band is a full-width slice of somebody else's panel, and reopening on the way past would drop the panel on top of the target you were heading for. What happens when you let go is up to you: *Settings → Appearance → Interface → UI Workspace → **After Dragging From The Bottom Panel*** chooses between **Reopen It** (the default — the panel returns to the state the drag found, because a panel that closed itself and stayed closed reads as a glitch) and **Leave It Closed**, for the workflow where the drag *was* what the panel was open for and having it spring back over what you just dropped is the opposite of helpful. The choice is per-user and survives a restart. Either way, a drag that begins with the panel already closed never opens it.
