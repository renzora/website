...
> Good to know: only objects with a name are saved, and the editor leaves out runtime-only data (like rebuilt physics colliders and render handles) because the engine recreates it automatically when the scene loads. For the full technical breakdown, see [Project Structure](/docs/r1-alpha8/setup/project-structure) and [Components](/docs/r1-alpha8/engine-core/components).
