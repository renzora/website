...
- Basic `Transform` replication with snapshot interpolation. Adding the `Networked` marker to an entity auto-inserts Lightyear's `Replicate` (to all clients) plus an interpolation target. See [State Replication](/docs/r1-alpha8/multiplayer/replication) for details.
...
- [Multiplayer Overview](/docs/r1-alpha8/multiplayer/overview) — the big picture
- [State Replication](/docs/r1-alpha8/multiplayer/replication) — the `Networked` marker and `Transform` sync
- [Lua](/docs/r1-alpha8/scripting/lua) — the full scripting surface (networking is Lua-only)
- [Exporting](/docs/r1-alpha8/exporting/overview) — building the binary you run with `--server`
