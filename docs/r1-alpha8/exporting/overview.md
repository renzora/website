...
`--host` instead runs a windowed listen server (client + server in one process). See [Server setup](/docs/r1-alpha8/multiplayer/server-setup) for the full flag list and deployment notes.
...
- [Installation → Working from a checkout](/docs/r1-alpha8/getting-started/installation) — the `renzora` CLI and Docker cross-compile setup behind these builds.
- [Multiplayer → Server setup](/docs/r1-alpha8/multiplayer/server-setup) — running the exported dedicated server.
