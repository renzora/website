...
The store is **scoped by engine version** on purpose. The runtime and the editor are two halves of one version, so a `r1-alpha8` editor picking up a `r1-alpha7` runtime would produce a game that fails to load the scene the editor just saved. Separate directories make that impossible rather than merely unlikely.
...
2. **A release tagged exactly `r1-alpha8`** — the normal case once the version has shipped.
3. **The newest nightly for this version** (`r1-alpha8-nightly-16aug26`) — the case for a build from source, whose version has no release yet. The export modal labels this "Nightly:" rather than "Release:" so the fallback is visible.
...
- `RENZORA_RELEASE_TAG` — the exact tag (`r1-alpha8`, or `r1-alpha8-nightly-16aug26`). Absent in a build from source, which is what makes it a *dev* build.
...
| Trigger | Tag | Kind |
|---|---|---|
| Nightly schedule (02:00 UTC) | `r1-alpha8-nightly-16aug26` | Prerelease, one per night, 14 kept |
| Push of an `r1-alpha*` tag | `r1-alpha8` | Full release |
...
- [Export Overview](/docs/r1-alpha8/exporting/overview) — the end-to-end export workflow and `.rpak`/VFS details.
- [Export: Android](/docs/r1-alpha8/exporting/android) — Android specifics (Gradle config, flavors, signing).
- [Building from source](/docs/r1-alpha8/setup/building-from-source) — cargo aliases and the Docker cross-compile image.
