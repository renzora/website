...
This page documents the surface registered by `crates/renzora_scripting` plus the functions declared by domain crates. For a guided introduction see [Lua](/docs/r1-alpha8/scripting/lua) and the [Scripting Overview](/docs/r1-alpha8/scripting/overview).
...
> There is **no** `on_start`, `on_collision`, or `on_destroy` hook. Use `on_ready` for setup and read the `is_colliding` global for overlap state. Collision *events* exist only as [Blueprint](/docs/r1-alpha8/scripting/blueprints) nodes.
...
This is the right tool for a gauge, a minimap, a radar, a health arc, a custom graph — anything that is drawing rather than layout. For a HUD made of *widgets*, use [Game UI](/docs/r1-alpha8/scripting/game-ui) markup instead.
...
See [Input Handling — Multiple gamepads](/docs/r1-alpha8/scripting/input#multiple-gamepads).
...
Native only. See [Multiplayer](/docs/r1-alpha8/multiplayer/overview).
...
Visual [Blueprints](/docs/r1-alpha8/scripting/blueprints) (`.blueprint` / `.bp`, JSON-serialized `BlueprintGraph`) **compile to Lua** and run through the script VM. There is no live graph interpreter — compilation is the single execution path, the way Unreal compiles its Blueprints to bytecode, so a blueprint and a hand-written script are the same thing by the time they execute.
...
- [Lua](/docs/r1-alpha8/scripting/lua) — guided introduction to the backend
- [Rust Scripts](/docs/r1-alpha8/scripting/rust-scripts) — `&mut World` per entity
- [Visual Blueprints](/docs/r1-alpha8/scripting/blueprints) — node graphs interpreted at runtime
- [Input Handling](/docs/r1-alpha8/scripting/input) — the action map and key names
- [Game UI](/docs/r1-alpha8/scripting/game-ui) — markup, `ui_*` verbs, and bindings
- [Script API Bindings](/docs/r1-alpha8/extending/script-bindings) — declaring a new function from a domain crate
- [Script Backends](/docs/r1-alpha8/extending/script-backends) — adding a language
