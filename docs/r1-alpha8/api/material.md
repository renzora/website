...
A Renzora material is a **node graph**, not a fixed list of PBR sliders. You build it in the [Material Editor](/docs/r1-alpha8/editor/materials) (the **Materials** workspace), and it is saved to disk as a `.material` file — JSON-serialized [`MaterialGraph`](#the-material-file-format). At runtime the `renzora_shader` crate's `MaterialResolverPlugin` watches every entity that has a `MaterialRef` component pointing at a `.material` (or `.shader`) file, compiles the graph, and applies the result to the mesh.
...
> For the **per-node reference** — every node's inputs, outputs, and exactly what it computes — see [Material Node Reference](/docs/r1-alpha8/api/material-node-reference). The tables below are the quick index.
...
To add your own node types to the graph, see [Custom Material Nodes](/docs/r1-alpha8/extending/material-nodes).
