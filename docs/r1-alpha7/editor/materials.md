...
**The Material panel** (the tab next to the preview, on the left) is the same values in list form. Click a node and the panel shows its name, what it does, and one labelled row per input pin — handy when a node's pins aren't obvious from their dots alone, or when you're zoomed out too far to read the node itself. Pins that already have a cable read *(connected)*. It's the same value either way, so editing in the panel updates the node and vice versa. The top of the panel also holds the material's **Name** and its **Domain**.
