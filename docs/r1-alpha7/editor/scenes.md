...
**Don't want to spend a row on it?** *Settings → Appearance → Interface → UI Workspace → Document Tabs* switches the strip for a **Top Bar Dropdown**: a single button beside **Play** showing the document you're in — its icon, its name, and the `*` when it has unsaved edits — that opens onto all of them, with **New Scene** at the bottom. The row goes back to the dock. The viewport's **maximize** button comes along to sit beside it, since it normally lives at the end of the strip. The choice is per-user (`~/.renzora/editor.toml`), so it survives restarts and doesn't travel with the project.
