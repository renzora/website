...
use renzora_common::ssr::ProfileSsr;
...
pub fn ProfilePage() -> impl IntoView {
    ...
    let head = ssr.clone().map(|p| {
        ...
        let canonical = format!("https://renzora.com/profile/{}", p.username);
        view! {
            <Title text=title />
    ...
    let ssr_heading = ssr.map(|p| view! {
        <div class="max-w-5xl mx-auto px-6 pt-6">
            <h1 class="text-2xl font-bold text-white">{p.username.clone()}</h1>
            {(p.role != "user").then(|| view! { <p class="text-sm text-accent mt-1">{p.role.clone()}</p> })}
        </div>
    });
    view! {
        {head}
        <section class="min-h-screen relative overflow-hidden">
            ...
            <div class="fixed bottom-20 left-1/4 w-[400px] h-[400px] bg-secondary/10 rounded-full blur-[120px] pointer-events-none"></div>

            <div id="pf-loading" class=if has_ssr { "hidden" } else { "flex items-center justify-center py-32" }>
            ...
            <div id="pf-content" class=if has_ssr { "" } else { "hidden" }>{ssr_heading}</div>

            // Edit modal
            <div id="pf-edit-overlay" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                <div class="w-[480px] max-w-[92vw] bg-surface-card border border-white/[0.08] rounded-2xl p-6 shadow-2xl">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div><label class="block text-[10px] text-zinc-500 mb-1">"Profile Color"</label><input id="ed-pc" type="color" value="#a855f7" class="w-full h-9 rounded-xl cursor-pointer bg-transparent border border-white/[0.06]" /></div>
        ...
        <script>
            ...
            (async function() {
                ...
                // Banner gradient (respects custom colors, defaults to the purple→cyan palette)
                ...
                let actions = '';
                if (!isOwn && token) {
                    actions += `<button onclick="window._follow('${p.username}')" class="px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${p.is_following?'bg-white/[0.06] border border-white/[0.08] text-zinc-300 hover:text-red-400':'bg-accent text-white hover:bg-accent-hover'}">${p.is_following?'Unfollow':'Follow'}</button>`;
                    actions += `<button onclick="window._friend('${p.username}')" id="pf-friend-btn" class="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.06] border border-white/[0.08] text-zinc-300 hover:text-white transition-all"><i class="ph ph-user-plus mr-1"></i>Add Friend</button>`;
                    actions += `<button onclick="window._msg('${p.id}')" class="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.06] border border-white/[0.08] text-zinc-300 hover:text-white transition-all"><i class="ph ph-chat-circle-dots mr-1"></i>Message</button>`;
                ...
                if (isOwn) {
                    actions += `<button onclick="window._openEdit()" class="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/[0.06] border border-white/[0.08] text-zinc-300 hover:text-white transition-all"><i class="ph ph-pencil-simple mr-1"></i>Edit</button>`;
                }
                ...
                const socialHtml = (p.connections||[]).map(c =>
                    `<a href="${c.platform_url||'#'}" target="_blank" class="w-8 h-8 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-zinc-400 hover:text-white hover:bg-white/[0.08] transition-all" title="${c.platform}"><i class="ph ${si[c.platform]||'ph-link'}"></i></a>`
                ).join('');

                // Badges
                const badgesHtml = p.badges.length ? p.badges.map(b =>
                    `<div class="flex items-center gap-2 p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.1] transition-all" title="${b.description}"><div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:${b.color}15"><i class="ph ${b.icon} text-lg" style="color:${b.color}"></i></div><div><div class="text-xs font-medium" style="color:${b.color}">${b.name}</div><div class="text-[9px] text-zinc-600">${b.description}</div></div></div>`
                ...
                <div class="max-w-5xl mx-auto px-4 sm:px-6 py-6 relative z-10">
                    ...
                    <!-- Banner hero card -->
                    <div class="rounded-2xl overflow-hidden border border-white/[0.08] mb-5">
                        <div class="h-32 sm:h-40 relative" style="background:linear-gradient(120deg, ${g1}, ${g2})">
                            <div class="absolute inset-0" style="background:radial-gradient(120% 130% at 82% -25%, rgba(34,211,238,0.35), transparent 60%)"></div>
                            <div class="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent"></div>
                        </div>
                        <div class="px-5 sm:px-7 pb-6">
                            <div class="flex flex-col sm:flex-row gap-5">
                                <!-- Avatar -->
                                <div class="w-28 h-28 sm:w-32 sm:h-32 -mt-14 sm:-mt-16 rounded-2xl overflow-hidden border-4 border-[#0b0617] bg-[#0e0e16] relative shrink-0 shadow-2xl shadow-black/50">
                                    ${avatar}
                                </div>
                                <!-- Name + meta + actions -->
                                <div class="flex-1 min-w-0 pt-1 sm:pt-4">
                                    <div class="flex items-start justify-between gap-3 flex-wrap">
                                        <div class="min-w-0">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <h1 class="text-2xl font-bold text-white">${p.username}</h1>
                                            ...
                                            ${p.bio ? `<p class="text-sm text-zinc-400 mt-1.5 leading-relaxed max-w-xl">${p.bio}</p>` : ''}
                                            <div class="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-[11px] text-zinc-500">
                                                ${p.location ? `<span class="flex items-center gap-1"><i class="ph ph-map-pin"></i>${p.location}</span>` : ''}
                                                ${p.website ? `<a href="${p.website}" target="_blank" class="flex items-center gap-1 text-secondary hover:underline"><i class="ph ph-link"></i>${p.website.replace('https://','').replace('http://','')}</a>` : ''}
                                    ...
                                    <!-- XP bar -->
                                    <div class="mt-4 flex items-center gap-2 text-xs max-w-lg">
                                        <span class="font-bold text-accent whitespace-nowrap">Lv ${lvl}</span>
                                        <div class="flex-1 h-2 bg-black/40 border border-white/[0.06] rounded-full overflow-hidden">
                                            <div class="h-full bg-gradient-to-r from-accent to-secondary rounded-full relative" style="width:${pct}%">
                                        ...
                                        <span class="text-[10px] text-zinc-500 whitespace-nowrap">${xp.toLocaleString()} / ${nxtXp.toLocaleString()} XP</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Stats row -->
                    <div class="grid ${(p.total_donated||0) > 0 ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-5' : 'grid-cols-2 sm:grid-cols-4'} gap-3 mb-5">
                        <div class="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 text-center">
                            <div class="text-2xl font-bold text-white">${(p.follower_count||0).toLocaleString()}</div>
                            <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-wide">Followers</div>
                        </div>
                        <div class="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 text-center">
                            <div class="text-2xl font-bold text-white">${(p.following_count||0).toLocaleString()}</div>
                            <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-wide">Following</div>
                        </div>
                        <div class="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 text-center">
                            <div class="text-2xl font-bold text-white">${(p.asset_count||0).toLocaleString()}</div>
                            <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-wide">Assets</div>
                        </div>
                        <div class="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 text-center">
                            <div class="text-2xl font-bold text-accent">${xp.toLocaleString()}</div>
                            <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-wide">Total XP</div>
                        </div>
                        ${(p.total_donated||0) > 0 ? `
                    ...
                    <!-- Badges -->
                    <div class="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5 mb-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">${badgesHtml}</div>
                    </div>
                ...
                if (isOwn) {
                    ...
                    document.getElementById('ed-pc').value = p.profile_color || '#a855f7';
            ...
            function ptab(name) {
                ...
                if (name === 'assets') {
                    assetPage = 0;
                    assetHasMore = true;
                    el.innerHTML = '<div id="pf-assets-grid" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3"></div><div class="flex justify-center mt-4" id="pf-assets-more-wrap"><button id="pf-assets-more" onclick="window._loadMoreAssets()" class="px-5 py-2 rounded-xl text-sm font-medium bg-white/[0.06] border border-white/[0.08] text-zinc-300 hover:text-white hover:bg-white/[0.08] transition-all">Load More</button></div>';
                    loadAssets();
                } else {
                ...
                try {
                    ...
                    if (Array.isArray(posts) && posts.length) {
                        el.innerHTML = posts.map(function(p) {
                            ...
                            return '<div class="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] mb-2"><p class="text-sm text-zinc-300">' + div.innerHTML + '</p><div class="flex gap-4 mt-2 text-xs text-zinc-600"><span><i class="ph ph-heart"></i> ' + p.like_count + '</span><span><i class="ph ph-chat-circle"></i> ' + p.comment_count + '</span><span>' + ago + '</span></div></div>';
                        }).join('');
