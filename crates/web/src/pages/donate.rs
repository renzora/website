...
pub fn DonatePage() -> impl IntoView {
    view! {
        ...
        <section class="max-w-4xl mx-auto py-12 px-4">
            ...
            <div id="goal-card" class="hidden bg-surface-card border border-zinc-800 rounded-2xl p-6 mb-8">
                <div class="flex items-baseline justify-between flex-wrap gap-2 mb-3">
                    <h2 id="goal-title" class="text-base font-semibold text-zinc-100"></h2>
                    <div class="text-sm text-zinc-400"><span id="goal-current" class="font-bold text-accent"></span>" / "<span id="goal-target"></span>" credits this month"</div>
                </div>
                <div class="h-4 rounded-full bg-zinc-800 overflow-hidden border border-zinc-700/50">
                    <div id="goal-bar" class="h-full bg-gradient-to-r from-accent to-purple-500 rounded-full transition-all duration-700" style="width:0%"></div>
                </div>
                <div class="flex items-center justify-between mt-2">
                    <p id="goal-desc" class="text-xs text-zinc-500"></p>
                    <span id="goal-percent" class="text-xs font-semibold text-zinc-400"></span>
                </div>
            </div>

            // Compact stats
            <div class="grid grid-cols-3 gap-3 mb-12">
                <div class="rounded-xl border border-white/[0.06] bg-[#0e0e16]/60 p-4 text-center">
                    <div id="stat-month" class="text-2xl font-bold text-accent">"..."</div>
                    <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-widest">"This Month"</div>
                </div>
                <div class="rounded-xl border border-white/[0.06] bg-[#0e0e16]/60 p-4 text-center">
                    <div id="stat-donors" class="text-2xl font-bold text-zinc-100">"..."</div>
                    <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-widest">"Supporters"</div>
                </div>
                <div class="rounded-xl border border-white/[0.06] bg-[#0e0e16]/60 p-4 text-center">
                    <div id="stat-alltime" class="text-2xl font-bold text-zinc-100">"..."</div>
                    <div class="text-[10px] text-zinc-500 mt-0.5 uppercase tracking-widest">"All-Time"</div>
                </div>
            </div>
            ...
            <div id="donate-form" class="hidden bg-surface-card border border-zinc-800 rounded-2xl p-6 mb-6 scroll-mt-24">
                ...
                <p class="text-xs text-zinc-500 mb-4">"Donate this month to appear on the supporters wall. Your tier is set by your month-to-date total."</p>
            ...
            // Recognition tiers
            <h2 class="text-xl font-bold text-center text-zinc-100 mb-1">"Recognition Tiers"</h2>
            <p class="text-xs text-zinc-500 text-center mb-6">"Based on how much you donate in a calendar month. Click a tier to donate that amount."</p>
            <div id="tier-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4"></div>
            <div class="text-center mb-12">
                <button onclick="toggleCorporate()" class="px-5 py-2.5 rounded-xl text-sm font-medium bg-zinc-800/50 border border-zinc-700 text-zinc-200 hover:border-accent/50 transition-colors">"Corporate Tiers"</button>
                <div id="corp-grid" class="hidden grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 text-left"></div>
            </div>

            // Sponsor listing editor
            <div id="sponsor-editor" class="hidden bg-surface-card border border-zinc-800 rounded-2xl p-6 mb-12">
                <h2 class="text-base font-semibold text-zinc-200 mb-1">"Your Supporter Listing"</h2>
            ...
            // Supporters wall
            <h2 class="text-2xl font-bold text-center text-zinc-100 mb-2">"This Month's Supporters"</h2>
            <div id="sponsor-wall" class="mb-10">
                <div class="flex justify-center py-4"><span class="loading loading-spinner loading-sm text-accent"></span></div>
            </div>

            // Past donators
            <div id="past-donors-section" class="hidden mb-14">
                <h3 class="text-lg font-bold text-center text-zinc-300 mb-4">"Past Donators"</h3>
                <p id="past-donors" class="text-[13px] text-zinc-500 leading-relaxed text-center max-w-3xl mx-auto"></p>
            </div>
            ...
            <div class="border-t border-zinc-800 pt-10">
                <div class="bg-surface-card border border-zinc-800 rounded-2xl p-6">
                    <h2 class="text-base font-semibold text-zinc-200 mb-4">"Top Donors (all-time)"</h2>
        ...
        var TIERS = [
            ...
            { slug: 'titanium', name: 'Titanium', credits: 1000, color: '#9ba1a6', perks: ['Name + link'] },
            { slug: 'diamond',  name: 'Diamond',  credits: 2500, color: '#7dd3fc', perks: ['Logo + link'] }
        ];
        var CORP_TIERS = [
            ...
            { slug: 'corp_gold',   name: 'Corporate Gold',   credits: 25000, color: '#ffd700', perks: ['Largest logo', 'Top billing'] }
        ];
        var WALL_ORDER = ['corp_gold', 'corp_silver', 'corp_bronze', 'diamond', 'titanium', 'platinum', 'gold', 'silver', 'bronze', 'supporter'];
        var TIER_NAMES = { supporter: { name: 'Supporters', credits: 0, color: '#a1a1aa' } };
        TIERS.concat(CORP_TIERS).forEach(function(t) { TIER_NAMES[t.slug] = t; });

        var token = document.cookie.match('(^|;)\\s*token\\s*=\\s*([^;]+)')?.pop();

        function usd(c) { return '$' + (c * 0.1).toLocaleString(undefined, {maximumFractionDigits: 0}); }
        ...
        function tierCard(t) {
            ...
            return '<button onclick="pickTier(' + t.credits + ')" class="text-left p-5 bg-white/[0.02] border border-zinc-800/70 rounded-2xl hover:border-accent/50 transition-all">' +
                '<div class="flex items-center gap-3">' +
                    ...
                    '</div>' +
                    '<div><div class="text-lg font-bold" style="color:' + t.color + '">' + t.name + '</div>' +
                    '<div class="text-xs text-zinc-400">' + t.credits.toLocaleString() + '+ credits <span class="text-zinc-600">(' + usd(t.credits) + ')</span></div></div>' +
                '</div>' +
                (perkChips ? '<div class="flex flex-wrap gap-1.5 mt-3">' + perkChips + '</div>' : '') +
            '</button>';
        }

        function toggleCorporate() { document.getElementById('corp-grid').classList.toggle('hidden'); }

        function pickTier(credits) {
            var input = document.getElementById('donate-amount');
            if (!input) { window.location.href = '/login'; return; }
            input.value = credits;
            document.getElementById('donate-form').scrollIntoView({ behavior: 'smooth', block: 'center' });
            input.focus();
        }

        document.getElementById('tier-grid').innerHTML = TIERS.map(tierCard).join('');
        document.getElementById('corp-grid').innerHTML = CORP_TIERS.map(tierCard).join('');

        (async function init() {
            fetch('/api/credits/donate/sponsors').then(r => r.json()).then(data => {
                var s = data.stats || {};
                document.getElementById('stat-month').textContent = (s.this_month || 0).toLocaleString();
                document.getElementById('stat-donors').textContent = (s.donors || 0).toLocaleString();
                document.getElementById('stat-alltime').textContent = (s.all_time || 0).toLocaleString();
                renderGoal(data.goal);
                renderWall(data.sponsors || []);
                renderPastDonors(data.past_donors || []);
            }).catch(() => {
        ...
        function renderGoal(goal) {
            ...
            document.getElementById('goal-title').textContent = goal.title || 'Community Goal';
            document.getElementById('goal-current').textContent = (goal.current || 0).toLocaleString();
            document.getElementById('goal-target').textContent = (goal.target || 0).toLocaleString();
            document.getElementById('goal-desc').textContent = goal.description || '';
            var pct = goal.percent || 0;
            document.getElementById('goal-percent').textContent = pct + '%';
            setTimeout(function() { document.getElementById('goal-bar').style.width = Math.min(100, pct) + '%'; }, 100);
        }
        ...
        function renderWall(sponsors) {
            ...
            WALL_ORDER.forEach(function(slug) {
                ...
                if (t.credits > 0) html += '<p class="text-xs text-zinc-500 mb-4">' + t.credits.toLocaleString() + '+ credits this month</p>';
                else html += '<p class="text-xs text-zinc-500 mb-4">This month</p>';
                ...
                if (isCorp) {
                    ...
                    group.forEach(function(s) {
                        ...
                        } else { html += '<span class="text-2xl font-bold text-zinc-100">' + esc(s.name) + '</span>'; }
                        html += '</a>';
                    });
                    html += '</div>';
                    ...
                    group.forEach(function(s) { html += linkOpen(s, 'text-[15px] text-accent hover:text-accent-hover transition-colors') + esc(s.name) + '</a>'; });
                    html += '</div>';
                } else {
                    var cls = (slug === 'platinum' || slug === 'gold') ? 'text-[14px] text-zinc-300' : 'text-[13px] text-zinc-400';
                    html += '<div class="flex flex-wrap justify-center gap-x-4 gap-y-1.5 max-w-2xl mx-auto">';
                    group.forEach(function(s) { html += '<span class="' + cls + '">' + esc(s.name) + '</span>'; });
                    html += '</div>';
                }
                html += '</div>';
            });
        ...
        function renderPastDonors(past) {
            if (!past.length) return;
            document.getElementById('past-donors-section').classList.remove('hidden');
            document.getElementById('past-donors').innerHTML =
                past.map(function(d) { return esc(d.name); }).join('<span class="text-zinc-700"> · </span>');
        }
        ...
        async function loadSponsorEditor() {
            try {
                ...
                if (!res.ok) return;
                var data = await res.json();
                if (!data.listed) return; // hasn't donated yet
        ...
        document.getElementById('sponsor-logo-file')?.addEventListener('change', async function() {
            ...
            } else { sponsorMsg(false, data.error || 'Upload failed'); }
        });
