            ...
            // Render one listing into `rootEl`.
            //
            // Both surfaces call this: the standalone page at
            // /marketplace/asset/<slug>, and the quick-look overlay on the
            // marketplace grid. It used to read the slug off the URL and write
            // into a hardcoded #asset-detail, which is exactly the coupling that
            // made a second surface impossible. Nothing else changed in the
            // move, so the overlay cannot drift from the page.
            //
            // Deliberately a classic script and not a module: every button this
            // renders uses an inline onclick, which resolves against the global
            // scope, so these declarations have to stay global.
            window.renderAssetDetail = async function(slug, rootEl) {
                ...
                // Served from the hover prefetch when the grid warmed this one,
                // which is the whole point of prefetching: the overlay opens on
                // data that is already in hand instead of a spinner.
                const res = await (window.assetDetailPrefetch?.take(slug)
                    ?? fetch('/api/marketplace/detail/' + slug, { headers: fetchHeaders }));
                if (!res.ok) { rootEl.innerHTML = '<p class="text-center text-zinc-500 py-20">Asset not found.</p>'; return; }
                ...
                el.innerHTML = `
                    ...
                    <!-- Hidden in the overlay, which has its own close affordance
                         and is already sitting on the grid this would navigate
                         to. On the standalone page it is the only way back. -->
                    <a href="/marketplace" class="detail-back inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-300 transition-colors mb-6">
